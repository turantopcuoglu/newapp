...
void main() {
  ...
  test('every category surfaces at least one recipe', () {
    ...
    // the home screen advertises a count for each one.
  ...
  test('the count shown on home is the list the user lands on', () {
    // Home and the detail screen both go through matchesSpecialCategory, so
    // this guards against them drifting apart into a lying badge.
    for (final category in specialCategories) {
      final homeCount =
          recipes.where((r) => matchesSpecialCategory(r, category)).length;
